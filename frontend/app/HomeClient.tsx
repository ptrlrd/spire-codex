...
const FALLBACK_DESCS: Record<string, string> = {
  cards:
    "Browse all Slay the Spire 2 cards. Filter by character, type, rarity, and keywords.",
  characters:
    "View all Slay the Spire 2 characters, stats, starting decks, relics, and NPC dialogues.",
  relics:
    "Explore all Slay the Spire 2 relics from starter to ancient tier. Filter by rarity and pool.",
  monsters:
    "Study all Slay the Spire 2 monsters, HP, moves, damage stats, and ascension scaling.",
  potions:
    "Discover all Slay the Spire 2 potions and their effects. Filter by rarity and character pool.",
  enchantments:
    "View all Slay the Spire 2 enchantments, effects, card type restrictions, and stackability.",
  encounters:
    "Browse all Slay the Spire 2 combat encounters across every act, normals, elites, and bosses.",
  events:
    "Explore all Slay the Spire 2 events, shrine events, Ancient encounters, choices, and outcomes.",
  powers:
    "Browse all Slay the Spire 2 powers, buffs, debuffs, and neutral status effects.",
  timeline:
    "Explore the Slay the Spire 2 timeline, epochs, story arcs, and unlockable content.",
  images:
    "Browse and download Slay the Spire 2 game art, card portraits, relic icons, monster sprites.",
  reference:
    "Slay the Spire 2 reference, keywords, orbs, afflictions, intents, acts, ascension, and more.",
  badges:
    "Run-end badges from Slay the Spire 2, Bronze, Silver, and Gold tier mini-achievements awarded on the Game Over screen.",
  guides:
    "Community strategy guides, character breakdowns, boss strategies, deckbuilding tips, and more.",
  leaderboards:
    "Fastest wins and highest ascensions from the community. Browse every submitted run.",
  submit:
    "Upload your .run files to contribute to leaderboards and community stats.",
  stats:
    "Win rates by character, card pick rates, most common relics, deadliest encounters.",
};
...
export default function HomeClient({
  ...
  const onGsScroll = () => {
    ...
    setGsIndex(
      Math.min(gsCount - 1, Math.max(0, Math.round(el.scrollLeft / step))),
    );
  };
  ...
  const langPrefix = LANG_CODES.has(pathLang)
    ? `/${pathLang}`
    : lang !== "eng"
      ? `/${lang}`
      : "";
  ...
  const ENGLISH_FALLBACKS = new Set(
    Object.values(SECTION_LABEL_MAP).map((v) => v.toLowerCase()),
  );
  const sectionKey = (key: string) => {
    ...
    if (
      gameT &&
      lang !== "eng" &&
      !ENGLISH_FALLBACKS.has(gameT.toLowerCase()) &&
      gameT.toLowerCase() !== key
    ) {
      return gameT;
    }
  ...
  const startTiles = [
    ...(!user
      ? [
          {
            title: t("Sign in with Steam", lang),
            desc: t(
              "Spire Codex keeps track of your runs and tier lists, all while not tracking you.",
              lang,
            ),
            href: loginSteam,
            ext: true,
      ...
      : []),
    {
      title: t("Download the app", lang),
      desc: t(
        "Download Spire Codex on Overwolf to upload your runs and get in-game help. The best Slay the Spire 2 companion app.",
        lang,
      ),
    ...
    {
      title: t("Download the mod", lang),
      desc: t(
        "Get the Spire Codex mod on the Steam Workshop for in-game stats contribution, auto run uploads, and a route planner.",
        lang,
      ),
    ...
    {
      title: t("Support on Patreon", lang),
      desc: t(
        "Like the project? Support us directly to unlock more features and keep the data free and as up to date as possible.",
        lang,
      ),
    ...
    {
      title: t("Create a tier list", lang),
      desc: t(
        "Show off your favorite tier lists of cards, relics, and more, and help others master the Spire.",
        lang,
      ),
    ...
    {
      title: t("Join the Discord", lang),
      desc: t(
        "Get on-demand updates, share tier lists, and show off your runs in the Spire Codex Discord.",
        lang,
      ),
    ...
    <>
      ...
      <section className="getstarted">
        <div className="gs-head">
          ...
          <p>
            {t(
              "Everything you need to climb the Spire. Track your runs, rank your favorites, and join the community.",
              lang,
            )}
          </p>
        </div>
        <div className="gs-grid" ref={gsTrack} onScroll={onGsScroll}>
          {startTiles.map((c) => {
            const inner = (
              <>
                <span className="act-ico">{c.icon}</span>
                <span className="act-t">{c.title}</span>
                <span className="act-d">{c.desc}</span>
              </>
            );
            return c.ext ? (
              <a
      ...
      <section className="hsec">
        <div className="s-head">
          ...
          <Link
            prefetch={false}
            className="viewmore"
            href={`${langPrefix}/characters`}
          >
            {t("All characters", lang)} {ARROW}
          </Link>
        </div>
        <div className="charbar">
          {CHARACTERS.map((char, i) => {
            const charName =
              translations.character_names?.[char.id] ??
            ...
            return (
              <Link
                ...
                className="charp"
                style={{ ["--cc"]: char.cssColor } as CSSProperties}
              >
                <span className="charp-art">
                  ...
                  <img
                    className="charp-combat"
                    src={imageUrl(
                      `/static/images/characters/combat_${char.id}.webp`,
                    )}
                  ...
                  />
                  <img
                    className="charp-icon"
                    src={imageUrl(
                      `/static/images/characters/character_icon_${char.id}.webp`,
                    )}
                    alt=""
                    aria-hidden="true"
                    width={88}
          ...
          {sections.map((s, i) => {
            const tile = (
              <Link
                ...
                className="btile"
                style={{ ["--cc"]: s.color } as CSSProperties}
              >
                <div className="bt-top">
                  <span className="bt-name">{sectionKey(s.key)}</span>
                  {s.count != null && (
                    <span className="bt-count">{s.count}</span>
                  )}
                </div>
              ...
              return [
                ...
                >
                  ...
                  <div className="promo-body">
                    <span className="promo-kick">
                      {t("New · Overwolf companion", lang)}
                    </span>
                    <div className="promo-t">
                      {t("Spire Codex Overlay", lang)}
                    </div>
                    <p className="promo-d">
                      {t(
                        "In-game lookup for cards, relics, monsters and events, plus a live run tracker that reads your save as you play.",
                        lang,
                      )}
                    </p>
                    <span className="promo-more">
                      {t("Learn more", lang)} &rarr;
                    </span>
                  </div>
                </Link>,
              ];
