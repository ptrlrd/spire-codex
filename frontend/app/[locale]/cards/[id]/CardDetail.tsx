...
function buildInteractiveWords(
  ...
  for (const kw of keywords) {
    ...
    words[kw] = { tooltip: desc, href: `${bp}/keywords/${kw.toLowerCase()}` };
  }
      ...
      <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="text-center py-12">
          ...
          <p className="text-[var(--text-muted)]">
            {t("No card exists with ID “{id}”.", { id })}
          </p>
        </div>
      </div>
    );
        ...
        <main className="main">
          ...
          <section id="description">
              ...
              <>
                ...
                <div className="kw-row">
                  {card.sources.map((s) => (
                    <Link
                      key={s.id}
                      href={`${bp}/monsters/${s.id.toLowerCase()}`}
                      className="kw"
                    >
                      {s.name}
                    </Link>
            ...
            {card.powers_applied && card.powers_applied.length > 0 && (
              <>
                <h3 className="subh">{t("Powers Applied")}</h3>
          ...
          <section id="relations">
            <h2>{t("Relations")}</h2>
            <p className="h-note">
              {t("What this card makes, and what else interacts with it.")}
            </p>
            <div className="rel">
              {spawnedCards.length > 0 && (
                <div className="rel-block">
                  <div className="rl">{t("Generates")}</div>
                  <div className="chips">
                    {spawnedCards.map((sc) => (
                      <Link
                        key={sc.id}
                        href={`${bp}/cards/${sc.id.toLowerCase()}`}
                        className="cardlink"
                      >
                        {sc.image_url && (
                          <img
          ...
          <EntityUpdateHistory entityType="cards" entityId={id} />
        </main>
            ...
            <div className="variant">
              {cardEnchantments.length > 0 && (
                <select
                  ...
                  aria-label={t("Enchantment")}
                  value={selectedEnch}
                  onChange={(e) => setSelectedEnch(e.target.value)}
                >
                  <option value="none">{t("No enchantment")}</option>
