...
export default function StatsClient({
    ...
    return (
      <div className="mx-auto max-w-[1400px] px-3 sm:px-5 py-6">
        <h1 className="text-3xl font-bold mb-2">
          <span className="text-[var(--accent-gold)]">{t("Stats")}</span>
        </h1>
  ...
  const TABS: { key: TopTab; label: string }[] = [
    ...
    { key: "encounters", label: t("Encounters") },
  ];
    ...
    <div className="mx-auto max-w-[1400px] px-3 sm:px-5 py-6">
      ...
      <div className="flex flex-wrap items-center gap-1.5 mb-4">
        ...
        {bracketActive && (
          <span className="text-xs text-[var(--text-muted)] ml-1">
            {t("Result and ascension filters don't apply within a bracket.")}
          </span>
        )}
      ...
      <div className="flex flex-wrap items-center gap-3 mb-6">
        <Pills
          ...
          value={players}
          onChange={setPlayers}
          ariaLabel={t("Filter by player count")}
        />
      </div>
      ...
      {!viewStats ? (
        <div className="text-center py-12 text-[var(--text-muted)]">
          {loading || bracketLoading ? t("Loading...") : t("No runs found.")}
        </div>
        ...
        <>
          ...
          {tab === "encounters" && <EncountersTab bp={bp} lang={lang} />}
        </>
      )}
    </div>
  );
      ...
      <div className="bg-[var(--bg-card)] rounded-xl border border-[var(--border-subtle)] p-5">
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 text-center">
          ...
          <div className="bg-[var(--bg-primary)] rounded-lg p-3">
            ...
            <div className="text-xs text-[var(--text-muted)]">{t("Win %")}</div>
          </div>
        </div>
      </div>
...
function EncountersTab({ bp, lang }: { bp: string; lang: string }) {
  const t = useT();
  ...
  if (ranked.length === 0) {
    return (
      <div className="bg-[var(--bg-card)] rounded-xl border border-[var(--border-subtle)] p-5 text-center text-sm text-[var(--text-muted)]">
        {t("No deadly encounters recorded.")}
      </div>
    );
  }
