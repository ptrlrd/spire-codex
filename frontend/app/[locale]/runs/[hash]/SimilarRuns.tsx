  ...
  return (
    <div className="bg-[var(--bg-card)] rounded-xl border border-[var(--border-subtle)] p-5">
      <h2 className="text-lg font-semibold text-[var(--text-primary)] mb-1">
        {t("Similar winning runs")}
      </h2>
      <p className="text-xs text-[var(--text-muted)] mb-3">
        {t("The closest winning decks to this one, by cards and relics.")}
      </p>
      {data.archetype && (
        <p className="text-xs mb-3">
          <span className="text-[var(--text-muted)]">{t("Build")}: </span>
          <Link
            href={`${bp}/archetypes`}
      ...
      {data.winners_also_took.length > 0 && (
        <div className="mt-4">
          <div className="text-xs font-semibold text-[var(--text-secondary)] mb-2">
            {t("Winners with decks like this also took")}:
          </div>
          <div className="flex flex-wrap gap-1.5">
            {data.winners_also_took.map((w) => (
              <Link
                prefetch={false}
                key={`${w.etype}-${w.id}`}
                href={`${bp}/${w.etype}/${w.id.toLowerCase()}`}
