...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  let title = `Slay the Spire 2 Deck Archetypes - Community Builds Ranked (sts2) | ${SITE_NAME}`;
  let description =
  ...
  if (locale !== "eng") {
    ...
    const heading = `${gameNameFor(locale)} ${t("Deck Archetypes")}`;
    title = `${heading} | ${SITE_NAME} (${LANG_NAMES[locale]})`;
    description = `${heading} (${LANG_NAMES[locale]}). ${t("Every deck archetype discovered from community runs: defining cards and relics, popularity, and real win rates per build.")}`;
  }
  return {
    title,
    description,
    ...
    openGraph: {
      title,
      description,
      url: `${SITE_URL}${localePath(locale, "/archetypes")}`,
      siteName: SITE_NAME,
      type: "website",
...
export default async function ArchetypesPage({ params }: Props) {
  ...
  const jsonLd = [
    buildBreadcrumbJsonLd([
      ...
      { name: t("Archetypes"), href: localePath(locale, "/archetypes") },
    ]),
  ];
