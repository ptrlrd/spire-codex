  ...
  return (
    <>
      ...
      <div className="bg-[var(--bg-card)] rounded-lg border border-[var(--border-subtle)] p-6 mb-6">
        <h1 className="text-2xl font-bold text-[var(--accent-gold)] mb-3">
          {guide.title}
        </h1>
        ...
        <div className="flex flex-wrap items-center gap-2 text-xs">
          <span className="text-[var(--text-muted)]">
            {t("by {author}", { author: guide.author })}
          </span>
          ...
          <span
            className={`px-2 py-0.5 rounded border text-[10px] font-medium ${difficultyColors[guide.difficulty] || ""}`}
          >
            {guide.difficulty}
          </span>
          <span className="px-2 py-0.5 rounded bg-[var(--bg-primary)] text-[var(--text-muted)] border border-[var(--border-subtle)] text-[10px]">
            {t(categoryLabels[guide.category] || guide.category)}
          </span>
          {guide.character && (
            <Link
              href={`${bp}/characters`}
              className="px-2 py-0.5 rounded bg-[var(--bg-primary)] text-[var(--accent-gold)] border border-[var(--border-subtle)] text-[10px] hover:border-[var(--accent-gold)] transition-colors"
            >
              {guide.character}
            </Link>
          )}
          {guide.tags
            .filter(
              (tag) =>
                tag.toLowerCase() !== guide.difficulty &&
                tag.toLowerCase() !== guide.category &&
                tag.toLowerCase() !== guide.character,
            )
            .map((tag) => (
              <span
                key={tag}
                className="px-2 py-0.5 rounded bg-[var(--bg-primary)] text-[var(--text-muted)] text-[10px]"
              >
                {tag}
              </span>
            ))}
      ...
      <div className="bg-[var(--bg-card)] rounded-lg border border-[var(--border-subtle)] p-6">
        <div className="guide-content" ref={contentRef}>
          <ReactMarkdown
            ...
            components={{
              h1: ({ children }) => (
                <h1 className="text-2xl font-bold text-[var(--accent-gold)] mt-8 mb-4 first:mt-0">
                  {children}
                </h1>
              ),
              h2: ({ children }) => (
                <h2 className="text-xl font-bold text-[var(--accent-gold)] mt-8 mb-3 first:mt-0">
                  {children}
                </h2>
              ),
              h3: ({ children }) => (
              ...
              h4: ({ children }) => (
                <h4 className="text-base font-semibold text-[var(--text-primary)] mt-4 mb-2">
                  {children}
                </h4>
              ),
              p: ({ children }) => (
              ...
              ol: ({ children }) => (
                <ol className="list-decimal list-inside text-sm text-[var(--text-secondary)] mb-4 space-y-1">
                  {children}
                </ol>
              ),
              li: ({ children }) => (
              ...
              blockquote: ({ children }) => (
                <blockquote className="border-l-2 border-[var(--accent-gold)] pl-4 my-4 text-sm text-[var(--text-muted)] italic">
                  {children}
                </blockquote>
              ),
              code: ({ children, className }) => {
                ...
                if (isBlock)
                  return (
                    <code
                      className={`block bg-[var(--bg-primary)] rounded-lg p-4 text-xs text-[var(--text-secondary)] overflow-x-auto mb-4 ${className}`}
                    >
                      {children}
                    </code>
                  );
                return (
                  <code className="bg-[var(--bg-primary)] px-1.5 py-0.5 rounded text-xs text-[var(--accent-gold)]">
                    {children}
                  </code>
                );
              },
              ...
              th: ({ children }) => (
                <th className="text-left font-semibold text-[var(--text-primary)] border-b border-[var(--border-subtle)] px-3 py-2">
                  {children}
                </th>
              ),
              td: ({ children }) => (
        ...
        <div className="border-t border-[var(--border-subtle)] mt-8 pt-5">
          <p className="text-xs text-[var(--text-muted)] text-center mb-3">
            {t("Written by {author}", { author: guide.author })}
          ...
          {(guide.website ||
            ...
            <div className="flex justify-center items-center gap-4">
              ...
              {guide.bluesky && (
                <a
                  href={
                    guide.bluesky.startsWith("http")
                      ? guide.bluesky
                      : `https://bsky.app/profile/${guide.bluesky}`
                  }
                  target="_blank"
                  rel="noopener noreferrer"
                  title="Bluesky"
                  className="text-[var(--text-muted)] hover:text-bluesky transition-colors"
                >
                  <svg
                    className="w-5 h-5"
                    viewBox="0 0 24 24"
                  ...
                  >
                    <path
                      d="M12 10.8c-1.087-2.114-4.046-6.053-6.798-7.995C2.566.944 1.561 1.266.902 1.565.139 1.908 0 3.08 0 3.768c0 .69.378 5.65.624 6.479.785 2.627 3.6 3.476 6.178 3.126-4.476.742-8.463 3.08-4.724 8.02 4.139 4.476 6.932-1.347 7.922-4.07.99 2.723 2.503 8.198 7.478 4.07 3.738-4.94-.249-7.278-4.724-8.02 2.578.35 5.392-.5 6.178-3.126C19.622 9.418 20 4.458 20 3.768c0-.69-.139-1.86-.902-2.203-.659-.3-1.664-.62-4.3 1.24C12.046 4.747 9.087 8.686 8 10.8h4z"
              ...
              {guide.twitter && (
                <a
                  href={
                    guide.twitter.startsWith("http")
                      ? guide.twitter
                      : `https://x.com/${guide.twitter}`
                  }
                  target="_blank"
                  rel="noopener noreferrer"
                  title="X / Twitter"
              ...
              {guide.twitch && (
                <a
                  href={
                    guide.twitch.startsWith("http")
                      ? guide.twitch
                      : `https://twitch.tv/${guide.twitch}`
                  }
                  target="_blank"
                  rel="noopener noreferrer"
                  title="Twitch"
                  className="text-[var(--text-muted)] hover:text-twitch transition-colors"
                >
                  <svg
                    className="w-5 h-5"
                    viewBox="0 0 24 24"
