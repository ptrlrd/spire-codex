  ...
  if (notFound || (!guide && !initialGuide)) {
    return (
      <div className="text-center py-20">
        <h1 className="text-2xl font-bold text-[var(--text-primary)] mb-2">{t("Guide Not Found")}</h1>
        <p className="text-[var(--text-muted)] mb-4">{t("This guide doesn't exist.")}</p>
        <Link href={`${bp}/guides`} className="text-[var(--accent-gold)] hover:underline">{t("Browse all guides")}</Link>
      </div>
    );
  }

  if (!guide) {
    return <div className="text-center py-20 text-[var(--text-muted)]">{t("Loading...")}</div>;
  }
      ...
      <div className="bg-[var(--bg-card)] rounded-lg border border-[var(--border-subtle)] p-6 mb-6">
        ...
        <div className="flex flex-wrap items-center gap-2 text-xs">
          ...
          {guide.updated && guide.updated !== guide.date && (
            <>
              <span className="text-[var(--text-muted)]">&middot;</span>
              <span className="text-[var(--text-muted)]">{t("Updated {date}", { date: guide.updated })}</span>
            </>
          )}
          ...
          {guide.character && (
            <Link href={`${bp}/characters`} className="px-2 py-0.5 rounded bg-[var(--bg-primary)] text-[var(--accent-gold)] border border-[var(--border-subtle)] text-[10px] hover:border-[var(--accent-gold)] transition-colors">
              {guide.character}
            </Link>
          )}
          {guide.tags
        ...
        <div className="border-t border-[var(--border-subtle)] mt-8 pt-5">
          ...
          {(guide.website || guide.bluesky || guide.twitter || guide.twitch) && (
            <div className="flex justify-center items-center gap-4">
              {guide.website && (
                <a href={guide.website} target="_blank" rel="noopener noreferrer" title={t("Website")} className="text-[var(--text-muted)] hover:text-[var(--accent-gold)] transition-colors">
