...
function GuidesClientInner({
  ...
  useEffect(() => {
    ...
    cachedFetch<GuideSummary[]>(`${API}/api/guides`)
      .then(setGuides)
      .catch(() => {});
  }, [initialGuides]);
  ...
  const updateUrl = useCallback(
    (newState: Record<string, string>) => {
      ...
      router.replace(`${bp}/guides${qs ? `?${qs}` : ""}`, { scroll: false });
    },
    [router, bp],
  );
    ...
    <>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {filtered.map((guide) => (
          ...
          >
            <h3 className="font-semibold text-lg text-[var(--accent-gold)] mb-1">
              {guide.title}
            </h3>
            ...
            <div className="flex flex-wrap items-center gap-2 text-xs">
              ...
              <span
                className={`px-2 py-0.5 rounded border text-[10px] font-medium ${difficultyColors[guide.difficulty] || "bg-surface text-fg-muted border-line-strong"}`}
              >
                {guide.difficulty}
              </span>
              ...
              {guide.tags
                ...
                .map((tag) => (
                  <span
                    key={tag}
                    className="px-1.5 py-0.5 rounded bg-[var(--bg-primary)] text-[var(--text-muted)] text-[10px]"
                  >
                    {tag}
                  </span>
                ))}
      ...
      {filtered.length === 0 && (
        <div className="text-center py-12 text-[var(--text-muted)]">
          {t("No guides found matching your filters.")}
        </div>
      )}
    </>
  );
...
export default function GuidesClient(
  props: Parameters<typeof GuidesClientInner>[0],
) {
  return (
