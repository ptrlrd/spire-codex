...
async function loadCodexNews(): Promise<{
  items: Announcement[];
  latestId: string;
}> {
  try {
    const res = await fetch(`${API}/api/news/codex`, {
      next: { revalidate: 300 },
    });
    if (res.ok) {
...
const TABS: {
...
}[] = [
  {
    key: "community",
    label: "Mega Crit",
    sublabel: "news_tab_community",
    feedType: 1,
  },
  {
    key: "codex",
    label: "Spire Codex",
    sublabel: "Site updates & new features",
    feedType: null,
  },
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return buildPageMetadata({
    locale,
    path: "/news",
    title: t("Patch Notes & Updates"),
    description: t("news_meta_description"),
  });
}
...
export default async function NewsPage({ params, searchParams }: Props) {
  ...
  const jsonLd = [
    ...
    buildCollectionPageJsonLd({
      ...
      items: items
        .slice(0, 50)
        .map((n) => ({ name: n.title, path: newsSlugForArticle(n.gid) })),
    }),
  ];
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <h1 className="text-3xl font-bold mb-2">
        <span className="text-[var(--accent-gold)]">{gameName(locale)}</span>{" "}
        {t("Patch Notes")} &amp; {t("News")}
      </h1>
      <p className="text-sm text-[var(--text-muted)] mb-6">
        Every Slay the Spire 2 (sts2) patch note, dev update, and announcement
        from Mega Crit, mirrored from Steam the moment it posts, plus press
        coverage from PCGamesN, RPS, and more.
        {latest
          ? ` The most recent update is ${latest.title} (${formatNewsDate(latest.date)}).`
          : ""}
      </p>
      ...
      <p className="text-xs text-[var(--text-muted)] mt-8">
        Article content © Mega Crit Games (Steam Community Announcements) and
        the respective publishers. Spire Codex mirrors and archives this feed
        for searchability, original links are preserved on every post.
      </p>
    </div>
  );
}
