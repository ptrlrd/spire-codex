...
import OrbDetail from "@/app/[locale]/orbs/[id]/OrbDetail";
...
async function fetchBeta(id: string) {
  const res = await fetch(`${API_INTERNAL}/api/orbs/${id}?channel=beta`);
...
export default async function Page({ params }: Props) {
  ...
  try {
    ...
    // OrbDetail refetches client-side (with channel=beta via the /beta
    // path) and renders its own not-found state.
  }
  return <OrbDetail initialOrb={item} />;
}
