      ...
      <div className="mstats">
        ...
        {move.powers && move.powers.length > 0 && (
          <div className="mrow eff">
            <span className="mk">{t("Effects")}</span>
            <div className="pills">
              {move.powers.map((p, i) => (
                <PowerPill key={i} p={p} powerData={powerData} bp={bp} />
              ))}
            </div>
          </div>
        )}
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="text-center py-12">
          ...
          <p className="text-[var(--text-muted)]">
            {t("No monster with id “{id}” exists.", { id })}
          </p>
        </div>
      </div>
    );
              ...
              <div className="enc-list mt-4">
                {monster.encounters!.map((enc) => (
                  <Link
                    ...
                    href={`${bp}/encounters/${enc.encounter_id.toLowerCase()}`}
                    className="enc-row"
                  >
