  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      {deckModifiers.length > 0 && (
        <div className="mb-6">
          ...
          <p className="text-xs text-[var(--text-muted)] mb-3">
            {t("These modifiers clear your starter deck and replace the Neow encounter. When active, Pandora's Box will not be offered by Darv.")}
          </p>
          <div className="space-y-3">
            {deckModifiers.map((mod) => (
              <ModifierCard key={mod.id} mod={mod} bp={bp} />
            ))}
          </div>
        </div>
      )}
      ...
      {neowModifiers.length > 0 && (
        <div className="mb-6">
          <h2 className="text-sm font-semibold text-[var(--text-muted)] uppercase tracking-wide mb-3">
            {t("Neow Replacement Modifiers")}
          </h2>
          <p className="text-xs text-[var(--text-muted)] mb-3">
            {t("These modifiers replace Neow's normal relic offerings with a custom selection.")}
          </p>
          <div className="space-y-3">
...
function ModifierCard({ mod, bp }: { mod: Modifier; bp: string }) {
