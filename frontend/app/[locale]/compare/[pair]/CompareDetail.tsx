    ...
    return (
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <p className="text-[var(--text-muted)]">
          {t("Could not load character data. Please try again later.")}
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="mb-2">
        ...
        >
          &larr; {t("All Comparisons")}
        </Link>
      </div>
      <h1 className="text-3xl font-bold mb-2">
        <span className={colorTextClass[colorA]}>{nameA}</span>
        <span className="text-[var(--text-muted)] mx-3 text-xl">{t("vs")}</span>
