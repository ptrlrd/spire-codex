...
export default function UninstallFormClient() {
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setStatus("error");
      setErrMessage(err instanceof Error ? err.message : t("Failed to submit"));
    }
  }
    ...
    return (
      <div className="text-center py-6">
        <h2 className="text-xl font-semibold text-[var(--accent-gold)] mb-3">
          {t("Thanks for using Spire Codex.")}
        </h2>
      ...
      <form onSubmit={onSubmit} className="space-y-6">
                ...
                {primary === reason && reason === REASON_SAVES && (
                  <div className="mt-2 pl-6">
                    <p className="text-sm text-[var(--text-primary)] mb-1.5">
                      {t("Did you read the FAQ and re-import your save?")}
                    </p>
                ...
                {primary === reason && FOLLOW_UP[reason] && (
                  <div className="mt-1.5 pl-6">
                    <textarea
                      ...
                      rows={2}
                      maxLength={1000}
                      placeholder={t(FOLLOW_UP[reason])}
                      aria-label={t(FOLLOW_UP[reason])}
                      className={field}
                    />
                  </div>
                )}
        ...
        <div>
          <label htmlFor="uninstall-improvement" className={heading}>
            {t("What's one thing we could do better?")}
          </label>
          <textarea
