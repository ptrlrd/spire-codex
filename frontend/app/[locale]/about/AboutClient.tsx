    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <div className="space-y-8">
        ...
        <div className="text-[var(--text-secondary)] leading-relaxed space-y-4">
          <p>
            {t("Spire Codex is a comprehensive database for Slay the Spire 2, commonly abbreviated StS2, built by reverse-engineering the game files. Every card, relic, monster, potion, event, and power on this site was extracted directly from the game's source code and localization data.")}
          </p>
          <p>
            {t("The project started from curiosity about how StS2 was built, and grew into a full API and website. The goal is to provide the kind of detailed, searchable reference that the Spire community deserves.")}
          </p>
          <p>
            ...
            {t(". This repo is downstream of where the project is hosted, so it may take a bit before your PR gets fully merged. The project has an")}{" "}
            <Link href="/developers" className="text-[var(--accent-gold)] hover:underline">
              {t("open API")}
            </Link>
            {" "}{t("that is free to use and self-hostable. And if you're wanting to chat about the project, come visit the")}{" "}
            <a
          ...
          <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-3">
            {STAT_ORDER.filter((s) => stats[s.key]).map((s) => (
              <div key={s.key} className="text-center">
                ...
                <div className="text-xs text-[var(--text-muted)]">{t(s.label)}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div>
          ...
          <p className="text-[var(--text-secondary)] mb-4">
            {t("Slay the Spire 2 is built with Godot 4, but all game logic lives in a C#/.NET 8 DLL. The data pipeline:")}
          </p>
          <div className="space-y-3">
            {PIPELINE_STEPS.map((step, i) => (
              ...
              >
                ...
                <div>
                  <div className="font-medium text-[var(--text-primary)] text-sm">{t(step.title)}</div>
                  <div className="text-sm text-[var(--text-secondary)]">{t(step.desc)}</div>
                </div>
              </div>
            ))}
          </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {[
              ...
              >
                <div className="font-medium text-[var(--text-primary)] text-sm mb-1">{t(f.title)}</div>
                <div className="text-xs text-[var(--text-secondary)]">{t(f.desc)}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-sm">
            {[
              ...
              <div key={row.label} className="bg-[var(--bg-card)] rounded-lg border border-[var(--border-subtle)] p-3">
                <div className="font-medium text-[var(--text-primary)] mb-1">{t(row.label)}</div>
                <div className="text-xs text-[var(--text-muted)]">{row.items}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <p className="text-xs text-[var(--text-muted)] border-t border-[var(--border-subtle)] pt-6">
          {t("This project is for educational purposes. All game data belongs to Mega Crit Games. This should not be used to recompile or redistribute the game.")}
        </p>
      </div>
    </div>
  );
