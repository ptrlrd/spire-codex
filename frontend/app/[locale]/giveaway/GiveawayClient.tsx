  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      <p className="text-[var(--text-secondary)] text-lg leading-relaxed mb-5">
        ...
        >
          {t("Artovision Slay the Spire 2 shadowbox")}
        </a>
        . {t("Enter free in three steps. The contest runs")} {t(WINDOW_LABEL)}.
      </p>
      ...
      >
        {loading ? (
          ...
          <p className="text-sm text-[var(--text-secondary)]">
            {t("Sign in with Steam below to start your entry.")}
          </p>
        ) : !hasSteam ? (
          <p className="text-sm text-[var(--text-secondary)]">
            {t("Your account is signed in, but you need a")} <b>Steam</b>{" "}
            {t("connection to enter. Connect Steam in step 1.")}
          </p>
        ) : (
          <p className="text-sm text-[var(--text-secondary)]">
      ...
      <div className="rounded-xl border border-dashed border-[var(--border-subtle)] bg-[var(--bg-card)]/50 p-5 mb-10">
        ...
        <p className="text-sm text-[var(--text-secondary)]">
          {t("Want in-game lookups and a live overlay too? Grab our")}{" "}
          <a
            href={OVERWOLF_URL}
            target="_blank"
            rel="noopener noreferrer"
