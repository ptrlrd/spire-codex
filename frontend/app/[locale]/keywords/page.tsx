...
function pageCopy(locale: Locale, t: TFn) {
  if (locale === "eng")
    return {
      heading: "Keywords & Game Terms",
      title: "Keywords & Game Terms - Slay the Spire 2 (sts2) | Spire Codex",
      description: "Every Slay the Spire 2 (sts2) card keyword and game term, Exhaust, Ethereal, Innate, Retain, Sly, Eternal, and more, with all cards using each.",
      tagline: "",
    };
  const gameName = gameNameFor(locale);
  const nativeName = LANG_NAMES[locale];
  const heading = `${gameName} ${t("Card Keywords")}`;
  const desc = `${heading} (${nativeName}). ${t("Every card keyword and game term, with all cards using each.")}`;
  return { heading, title: `${heading} | Spire Codex (${nativeName})`, description: desc, tagline: t("keywords_tagline") };
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const locale = localeOf((await params).locale);
  const copy = pageCopy(locale, await getT(locale));
  return listMetadata(locale, { path: "/keywords", title: copy.title, description: copy.description });
}
