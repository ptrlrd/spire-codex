...
function TickerRow({
  ...
  const meta = [e.turn != null ? t("T{n}", { n: e.turn }) : "", ago(e.t, t)]
    .filter(Boolean)
    .join(" · ");
...
function LiveCombatPanel({
  ...
  const dmg: [string, number | null | undefined][] = [
    [t("Damage dealt"), p.damage_dealt],
    [t("This turn"), p.damage_dealt_this_turn],
    [t("Damage taken"), p.damage_taken],
    [t("Biggest hit"), p.biggest_hit],
  ];
          ...
          {shownPiles.map(([label, v]) => {
            ...
            return hasCards ? (
              <button
                ...
                title={t("{label} pile — click to view", { label: t(label) })}
              ...
              <span
                key={label}
                title={t("{label} pile", { label: t(label) })}
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="text-sm text-[var(--text-muted)] truncate">
            ...
            {p.run_time != null
              ? ` · ${t("run {time}", {
                  time: `${Math.floor(p.run_time / 60)}:${String(
                    Math.floor(p.run_time % 60),
                  ).padStart(2, "0")}`,
                })}`
              : ""}
            {p.seed ? ` · ${t("seed {seed}", { seed: p.seed })}` : ""}
          </div>
