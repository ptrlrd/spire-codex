...
export default function TierListBuilder({ entityType, entities, initial }: Props) {
  ...
  async function handleExport() {
    ...
    try {
      ...
      setError(t("Could not export the image. Try again."));
    } finally {
      setExporting(false);
    }
  }
  ...
  return (
    <div className="mx-auto max-w-[1800px] px-4 sm:px-6 py-6">
      ...
      >
        <span aria-hidden>←</span> {t("Back to tier lists")}
      </Link>
      <div className="mb-4 flex flex-wrap items-center gap-3">
        ...
        >
          {exporting ? t("Exporting…") : t("Export image")}
        </button>
        <button
          onClick={handleSave}
        ...
        <div className="mt-6">
          <div className="mb-2 flex items-center justify-between gap-2">
            <h2 className="text-sm font-semibold uppercase tracking-wide text-[var(--text-secondary)]">
              {t(ENTITY_LABEL[entityType])} ({filteredTray.length})
            </h2>
