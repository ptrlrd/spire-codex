...
interface CommunityStats {
  ...
  beta?: {
    deaths?: {
      encounters?: { id: string; name: string; count: number }[];
  ...
  records: {
    fastest_win: Record_ | null;
    longest_run: Record_ | null;
    biggest_deck: Record_ | null;
  };
}
...
async function fetchList<T>(path: string, lang: Locale): Promise<T[]> {
  try {
    const res = await fetch(`${API_INTERNAL}${path}${langQuery(lang)}`, {
...
async function fetchCharacterNames(
  ...
  try {
    const res = await fetch(
      `${API_INTERNAL}/api/translations${langQuery(lang)}`,
      { next: { revalidate: 3600 } },
    );
    if (!res.ok) return {};
...
export async function CommunityStatsBody({
  ...
  for (const e of eventList) {
    ...
    for (const p of e.pages ?? [])
      for (const o of p.options ?? []) titles[o.id] ??= stripTags(o.title);
    choiceTitles[e.id.toUpperCase()] = titles;
  }
  const charName = (id: string) =>
    charNames[id.toLowerCase()] ?? characterName(id);
  const restLabel = (id: string) =>
    REST_KEYS[id.toUpperCase()]
      ? t(REST_KEYS[id.toUpperCase()])
  ...
  const choiceLabel = (eventId: string, choiceId: string, fallback: string) => {
    ...
    return (
      titles[choiceId] ?? titles[choiceId.replace(/_\d+$/, "")] ?? fallback
    );
  };
  ...
  return (
    <div className="mx-auto max-w-[1400px] px-3 sm:px-5 py-6">
      ...
      <p className="text-sm text-[var(--text-muted)] mb-8">
        {t("How the community actually plays")} <em>Slay the Spire 2</em>
        {t(", drawn from")} {stats.total_runs.toLocaleString()}{" "}
        {t(
          "submitted runs. A naive snapshot of the data, not a verdict on what is correct.",
        )}
      </p>
      ...
      <section className="mb-10">
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          ...
          <StatCard
            label={t("Wins")}
            value={stats.total_wins.toLocaleString()}
          />
          <StatCard
            label={t("Losses")}
      ...
      <section className="mb-10 grid grid-cols-1 md:grid-cols-3 gap-6">
        <div>
          ...
          <RankBars
            ...
            data={stats.by_character.map((c) => ({
              name:
                charNames[c.id.toLowerCase()] ?? c.name.replace(/^The\s+/i, ""),
              value: c.win_rate,
              display: `${c.win_rate}%`,
        ...
        {stats.by_ascension.length > 2 && (
          <div>
            <h2
              className="text-lg font-semibold text-[var(--accent-gold)] mb-3"
              title={t(
                "Win-rate change at each ascension step. The deepest bar is where the community hits the wall.",
              )}
            >
              {t("The ascension wall")}
            </h2>
            <RankBars
              vertical
              color={ROSE}
                    ...
                    : [
                        {
                          ...
                          value:
                            Math.round(
                              (a.win_rate - arr[i - 1].win_rate) * 10,
                            ) / 10,
      ...
      {Object.keys(stats.ascension_matrix || {}).length > 0 && (
        <section className="mb-10">
          <h2 className="text-lg font-semibold text-[var(--accent-gold)] mb-1">
            {t("Win rate by character and ascension")}
          </h2>
          <p className="text-sm text-[var(--text-muted)] mb-3">
            {t(
              "Every character at every ascension. The pale band is where the community wins; the dark band is the wall.",
            )}
          </p>
          <AscensionHeatmap
            matrix={stats.ascension_matrix!}
            lang={lang}
            names={charNames}
          />
        </section>
      ...
      {(stats.character_behavior?.length ?? 0) > 0 && (
        <section className="mb-10 grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <h2 className="text-lg font-semibold text-[var(--accent-gold)] mb-1">
              {t("Who removes the most")}
            </h2>
            <p className="text-sm text-[var(--text-muted)] mb-3">
              {t(
                "Cards removed per run at shops and events, by the removing player's character.",
              )}
            </p>
            {(() => {
                    ...
                    {rows.map((c) => (
                      ...
                      >
                        ...
                        <span className="tabular-nums text-[var(--text-muted)]">
                          ·{" "}
                          {(
                            Math.round((c.removes / total) * 1000) / 10
                          ).toFixed(1)}
                          %
                        </span>
                      </li>
          ...
          <div>
            ...
            <p className="text-sm text-[var(--text-muted)] mb-3">
              {t("What each character does at rest sites.")}
            </p>
            <div className="rounded-lg border border-[var(--border-subtle)] bg-[var(--bg-card)] p-4">
                ...
                {(stats.character_behavior ?? []).map((c) => (
                  ...
                  >
                    <EventDonut
                      ...
                      colors={Object.keys(c.rest).map(
                        (a) => REST_HEX[a] ?? OPTION_HEX[0],
                      )}
                    />
                    <CharacterTag
                      id={c.id}
      ...
      {(stats.survival?.length ?? 0) > 0 && (
        <section className="mb-10">
          ...
          <p className="text-sm text-[var(--text-muted)] mb-3">
            {t(
              "The share of runs still alive at each floor, abandons included.",
            )}
          </p>
          <SurvivalLine
            data={stats.survival ?? []}
      ...
      {/* Event decisions */}
      <section className="mb-10">
        <h2 className="text-xl font-semibold text-[var(--accent-gold)] mb-1">
          {t("How players vote")}
        </h2>
          ...
          {stats.events.map((raw) => {
            const e = {
              ...
              options: raw.options.map((o) => ({
                ...o,
                label: choiceLabel(raw.id, o.id, o.label),
              })),
            };
            return (
              <div
                    ...
                    {e.options.map((o, i) => (
                      ...
                      >
                        <span
                          className="inline-block w-2 h-2 rounded-sm"
                          style={{
                            backgroundColor: OPTION_HEX[i % OPTION_HEX.length],
                          }}
                        />
      ...
      <section className="mb-10 grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          ...
          <RankBars
            color={ROSE}
            data={rankPct(named(stats.deaths.encounters, encounterNames))}
          />
        </div>
        <div>
          <h2 className="text-lg font-semibold text-[var(--accent-gold)] mb-3">
          ...
          <RankBars
            color={ROSE}
            data={rankPct(named(stats.deaths.events, eventNames))}
          />
        </div>
      </section>
      ...
      {((stats.beta?.deaths?.encounters?.length ?? 0) > 0 ||
        (stats.beta?.deaths?.events?.length ?? 0) > 0) && (
        <section className="mb-10">
          <h2 className="text-lg font-semibold text-success mb-3">
            {t("From the beta branch")}
          </h2>
          <div className="rounded-lg border border-success/40 bg-success/10 p-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              {[
                ...(stats.beta?.deaths?.encounters ?? []),
                ...(stats.beta?.deaths?.events ?? []),
              ].map((e) => (
                <div
                  key={e.id}
                  className="flex items-center justify-between rounded bg-[var(--bg-card)] px-3 py-2 text-sm"
                >
                  <span className="text-[var(--text-primary)]">
                    {e.name}
                    <span className="ml-2 text-[10px] px-1.5 py-0.5 rounded-full font-semibold bg-success/15 text-success border border-success/30">
                      {t("Beta")}
                    </span>
                  </span>
                  <span className="text-[var(--text-secondary)] tabular-nums">
                    {e.count.toLocaleString()} {t("kills")}
                  </span>
                </div>
              ))}
            </div>
      ...
      <section className="mb-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          <RecordCard
            label={t("Fastest win")}
            value={fmtTime(records.fastest_win?.run_time)}
            hash={records.fastest_win?.run_hash}
            lang={lang}
          />
          <RecordCard
            label={t("Longest run")}
            value={fmtTime(records.longest_run?.run_time)}
            hash={records.longest_run?.run_hash}
            lang={lang}
          />
          <RecordCard
            ...
            value={
              records.biggest_deck
                ? `${records.biggest_deck.size} ${t("cards")}`
                : "-"
            }
            hash={records.biggest_deck?.run_hash}
            lang={lang}
          />
        </div>
      </section>
      ...
      <section className="mb-10 grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div>
          <h2 className="text-lg font-semibold text-[var(--accent-gold)] mb-3">
            {t("Most-removed cards")}
          </h2>
          <RankBars
            color={GOLD}
            data={rankCount(named(stats.most_removed, cardNames))}
          />
        </div>
        {(stats.hopper_stolen?.length ?? 0) > 0 && (
          <div>
            <h2 className="text-lg font-semibold text-[var(--accent-gold)] mb-3">
              {t("Stolen by the {monster}", {
                monster: monsterNames["THIEVING_HOPPER"] ?? "Thieving Hopper",
              })}
            </h2>
            <RankBars
              color={ROSE}
              data={rankCount(named(stats.hopper_stolen ?? [], cardNames))}
            />
          </div>
        )}
        <div>
          <h2 className="text-lg font-semibold text-[var(--accent-gold)] mb-3">
            {t("Favorite ancient relics")}
          </h2>
          <RankBars
            color={GOLD}
            data={rankPct(named(stats.ancient_picks, relicNames))}
          />
        </div>
        <div>
          <h2 className="text-lg font-semibold text-[var(--accent-gold)] mb-3">
            {t("Card reward skip rate")}
          </h2>
          <StatCard
            label={t("of card rewards skipped")}
            value={`${stats.reward_skip_rate}%`}
          />
        </div>
      </section>
      ...
      <p className="text-xs text-[var(--text-muted)]">
        ...
        {t("for how the data is gathered and where it is biased.")}
      </p>
    </div>
  );
}
