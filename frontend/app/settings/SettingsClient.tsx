  ...
  const disconnect = async (
    ...
    try {
      ...
      if (res.ok) {
        ...
        const err = await res.json().catch(() => null);
        toast(
          err?.detail || `${t("Failed to disconnect", lang)} ${label}`,
          "error",
        );
      }
    } catch {
    ...
    return (
      <div className="max-w-2xl mx-auto px-4 py-12 text-center">
        <h1 className="text-2xl font-bold text-[var(--text-primary)] mb-4">
          {t("Sign in to view settings", lang)}
        </h1>
        <p className="text-[var(--text-secondary)]">
          {t("Connect your Steam or Discord account.", lang)}
        </p>
      </div>
    );
  }
        ...
        <>
          ...
          <section className="space-y-3">
            ...
            <div className="flex flex-col sm:flex-row gap-2">
              <div className="flex-1 relative">
                ...
                {!checkingUsername &&
                  ...
                  usernameChanged && (
                    <span className="absolute right-3 top-2.5 text-xs text-red-400">
                      {t("Taken", lang)}
                    </span>
                  )}
                {!checkingUsername &&
                  ...
                  usernameChanged && (
                    <span className="absolute right-3 top-2.5 text-xs text-green-400">
                      {t("Available", lang)}
                    </span>
                  )}
              </div>
            ...
            <p className="text-xs text-[var(--text-tertiary)]">
              ...
              {changesRemaining} {t("changes remaining today.", lang)}
            </p>
          </section>
